...
Debian desktop profiles reuse this flow: `debian-xfce` selects `xfce-desktop`, configures LightDM autologin and a serial getty in `late_commands`, and checks `xfce4` plus the user-owned `xfce4-session` with `verify-desktop`. New profiles remain experimental until a clean live matrix PASS.
...
`kali` reuses `bootstrap-preseed`. Kali netinst preseed uses the vendor Xfce task selection and kali-rolling mirror, LightDM autologin and serial getty. Verification requires kali-desktop-xfce, an active local session and xfce4-session. SSH 2269; experimental pending clean live validation.
