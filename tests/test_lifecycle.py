...
class VmctlTests(BaseVmctlTestCase):
    ...
    def test_check_vms_restore_returns_disk_bytes_after_failed_install(self):
        ...
        disk.write_bytes(bytes(range(256)) * 16384)
        ...
        def fake_bootstrap(args: argparse.Namespace) -> int:
            ...
            disk.parent.mkdir(parents=True, exist_ok=True)
            disk.write_bytes(b"THROWAWAY TEST DISK")
        ...
        with mock.patch.object(vmctl.lifecycle, "cmd_bootstrap_unattended", side_effect=fake_bootstrap), \
             ...
             mock.patch.object(vmctl.lifecycle, "prepare_vm_for_local_test", return_value=(ubuntu_vm, None)), \
             mock.patch.object(vmctl.lifecycle, "local_test_prereq_skip", return_value=None):
            exit_code = self.vmctl.cmd_test_local(args)
