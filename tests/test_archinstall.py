...
class ArchinstallBootstrapTests(BaseVmctlTestCase):
    ...
    def test_render_bootstrap_script_contains_sgdisk_and_pacstrap(self):
        ...
        self.assertIn("grub-mkstandalone", script)
        ...
        self.assertIn("efibootmgr --create", script)
        self.assertIn("Arch Linux", script)
        self.assertIn("insmod part_gpt", script)
        self.assertIn("insmod ext2", script)
        self.assertIn("set root=(hd0,gpt2)", script)
        self.assertNotIn("search --no-floppy --fs-uuid --set=root", script)
