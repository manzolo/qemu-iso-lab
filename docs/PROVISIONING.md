...
| `autoinstall` (Ubuntu) | `hostname`, `username`, `realname`, `password_hash`, `timezone`, `keyboard_layout`, `storage_layout`, `install_ssh`, `packages`, `late_commands` (subiquity late-commands: the way to install packages the live-server CD does not carry on 20.04, whose subiquity resolves `packages` against the CD pool only, e.g. `curtin in-target --target=/target -- apt-get install -y ubuntu-desktop`) |
