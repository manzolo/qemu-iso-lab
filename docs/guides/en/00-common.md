...
make setup                # vmctl + vmtui into ~/.local/bin, installs what is missing (asks first), checks the host
...
`xorriso`/`cloud-localds` for seed ISOs, `7z` for the Windows ISO (UDF file system),
