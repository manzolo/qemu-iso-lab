...
| 25 | The same “Migrazione da WinSock 1.1 a 2.0 non riuscita (0xffffffff)” box as row 12, and a STOP 0x0A in `tcpip.sys` (`f3ea397e`, base `f3e9b000`) in the network stage | **Not row 12.** Reproduced on 2026-09-16 in two standalone runs, outside `check-vms`, and the guest registry of the failed install has what the patch is supposed to write: `Services\AMDPCN1\Parameters` = `TP 0`, `FDUP 0`, `MediaType 1`. Writing `TPValue = 0` outside the `STF_GUI_UNATTENDED` guard changed nothing and was reverted. The profile passed ten runs on 09-14/15 with the same medium, the same `windowsnt4.py` and no QEMU upgrade on the host since August, so this is a regression whose variable has not been found; the only environmental signal is the repeated `Slirp: Failed to send packet, ret: -1`, which also appears early in runs, not only at the stall | None yet. Next: why `amdpcn.sys` does not start with the parameters it asked for - a run with the adapter left uninstalled would say whether the stop follows the adapter or the stack |
